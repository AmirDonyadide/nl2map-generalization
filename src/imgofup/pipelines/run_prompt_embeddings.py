...
from imgofup.embeddings import prompts as pe
...
def run_prompt_embeddings_from_config(
    *,
    ...
    cfg: Any,
    paths: Any,
    verbosity: int = PROMPT_EMBED_VERBOSITY_DEFAULT,
    l2_normalize: bool = PROMPT_EMBED_L2_NORMALIZE_DEFAULT,
    also_save_embeddings_csv: bool = PROMPT_EMBED_SAVE_CSV_DEFAULT,
    ...
    pe.setup_logging(verbosity=int(verbosity))

    prompt_id_col = getattr(paths, "PROMPT_ID_COL", LABELS_EXCEL_PROMPT_ID_FALLBACK)
    ...
    pe.save_outputs(
        ...
        also_save_embeddings_csv=bool(also_save_embeddings_csv),
    )

    embeddings_path = out_dir / PROMPT_EMBEDDINGS_NPZ_NAME
    prompts_parquet_path = out_dir / PROMPTS_PARQUET_NAME
